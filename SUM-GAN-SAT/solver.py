...
class Solver(object):
    ...
    def build(self):
        ...
        if self.config.mode == 'train':
            # Build Optimizers
            self.s_e_optimizer = optim.Adam(list(self.summarizer.attn.parameters()) + list(self.summarizer.trans.parameters())
                + list(self.linear_compress.parameters()),
                lr=self.config.lr)

            self.d_optimizer = optim.Adam(list(self.summarizer.trans.parameters()) + list(self.linear_compress.parameters()),
                lr=self.config.lr)
    ...
    def train(self):
        ...
        for epoch_i in trange(self.config.n_epochs, desc='Epoch', ncols=80):
            ...
            for batch_i, image_features in enumerate(tqdm(self.train_loader, desc='Batch', ncols=80, leave=False)):
                ...
                _, uniform_features = self.summarizer(original_features, uniform=True)
                ...
                if batch_i > self.config.discriminator_slow_start:
                    ...
                    scores, generated_features = self.summarizer(
                        original_features)
                    _,  uniform_features = self.summarizer(
